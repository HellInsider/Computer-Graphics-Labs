...
cbuffer CommonConstBuf : register(b0)
{
  ...
  int IsPrefilteredTexture;
  int IsBRDFLutTexture;
};
...
float3 Shade2(float3 P, float3 N, float2 T)
{
  ...
  // for IBL
  if (IsIrradianceTexture && IsPrefilteredTexture && IsBRDFLutTexture)
  {
      static const float MAX_REFLECTION_LOD = 4.0;
      float3 prefilteredColor = PrefilteredTexture.SampleLevel(Sampler0, -R, roughness * MAX_REFLECTION_LOD);
      float3 F = F0 + (max((1.0 - roughness).xxx, F0) - F0) * pow(1.0 - nv, 5.0);
      float2 envBRDF = BRDFLUTTexture.Sample(Sampler0, float2(nv, roughness));
